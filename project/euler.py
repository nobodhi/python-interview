...
def find_primes(max_value):
    ...
    # todo use tuple instead of dict, almost as fast
...
def triangle_number_divisors(n) -> int:
    '''
    returns the first triangle number with at least n divisors
    '''
    # outer loop adds up the natural numbers and keeps sum
    t = 0  # the natural numbers
    s = t  # the sum of t
    d = 0  # the count of divisors of s
    while n >= d:
        t += 1
        s += t
        # inner loop finds divisors of each and breaks when it finds one with > n divisors
        d = 0
...
print("triangle_number_divisors ", triangle_number_divisors(500))
